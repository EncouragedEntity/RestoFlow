const authLoginUrl = "/api/warlock/auth/login";
const authRegisterUrl = "/api/warlock/auth/register";
const authUserAccount = "/api/warlock/public/account";
const restaurant = "/api/warlock/public/restaurant";
const dineProduct = "/api/dine/public/product";
const dineProductCategory = "$dineProduct/category";
const measurementUnit = "/api/dine/public/measurement/unit";
const dineTable = "/api/dine/public/table";
const payment = "/api/dine/public/payment";
const websocketEndpoint = "/api/dine/public/websocket-endpoint";
const history = "/api/dine/public/order/history";
const qrPrefix = "restoflow_";

const orderStatusReady = "READY";
const orderStatusChoosing = "CHOOSING";
const orderStatusOrdered = "ORDERED";
